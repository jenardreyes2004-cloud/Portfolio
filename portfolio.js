...
function renderProjects(filter = 'all') {
  ...
  grid.innerHTML = '';
  const categoryColors = { networking: '#1a3a5c', iot: '#5a2d4a', design: '#7a5c2d' };
  const categoryIcons  = { networking: 'diagram-2', iot: 'cpu', design: 'pentagon' };
      ...
      col.innerHTML = `
        <div class="feature-card p-3 h-100" style="display:flex;flex-direction:column;">
          ...
          <button class="btn btn-sm btn-outline-dark project-open"
            ...
            data-tags='${JSON.stringify(project.tags)}' data-image="${project.image||''}" data-image2="${project.image2||''}">View details</button>
        </div>
      `;
...
function exportCv() {
  ...
  const projRows = projects.map(p => `
    <div style="margin-bottom:8px;">
      ...
      <div style="font-size:10px;color:#888;">${p.tags.join(' · ')}</div>
    </div>
  `).join('');
